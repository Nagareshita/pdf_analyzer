    ...
    def __init__(
        ...
        **kwargs
        ...
        if llm_config is None:
            ...
            logger.info('llm_config is None. Initializing the Qwen3Config with default values (`Qwen3Config`).')

        self.vision_config = SAILViTConfig(**vision_config)
        arch = llm_config.get('architectures', [''])[0]
        if arch == 'LlamaForCausalLM':
            self.llm_config = LlamaConfig(**llm_config)
        elif arch in ('Qwen3ForCausalLM', 'Qwen2ForCausalLM'):
            self.llm_config = Qwen3Config(**llm_config)
        else:
            logger.warning(f'Unsupported architecture "{arch}", falling back to Qwen3Config.')
            self.llm_config = Qwen3Config(**llm_config)
